---
title: "Detecting mistakes in weight-lifting exercieses by using activity recognition techniques"
author: "Lance Bryant"
date: "July 19, 2014"
output:
  html_document:
...
Inspection of the data reveals that many variables consist of a high number of `NA` values. We remove these variables from the dataset along with variables that have no bearing on the exercises (user name, time of meausurement, etc.)
...
Below we disply two confusion matrices and related statistics. The first shows that the in-sample accuracy of our model is 100%. The second shows the out-of-sample accuracy is between 98.7% and 99.2% at the 95%-confidence level.
